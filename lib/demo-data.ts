...
export const DEMO_JOB: JobProfile = {
  ...
  description:
    "Own AI products from ambiguous idea to reliable production release. Design agentic systems that combine LLMs, RAG, MCP, deterministic workflows, guardrails, and human approval; build the Python or equivalent backend APIs, data stores, vector retrieval, automation, monitoring, and usable product interface needed to make them operational. Strong evidence includes production AI architecture, FastAPI or Django services, PostgreSQL or equivalent databases, Redis, vector databases, Docker, Kubernetes, CI/CD, observability, n8n-style automation, and secure enterprise integrations. The role values a pragmatic builder who can choose managed services where useful, explain trade-offs, test failure modes, monitor drift and reliability, and ship independently under incomplete requirements. React, Next.js, or TypeScript experience is useful, but equivalent full-stack delivery and the ability to learn the necessary interface stack quickly are accepted. Research depth is valuable when it translates into maintainable systems, measurable product outcomes, and clear communication.",
};
